...
public class skeletonAnimator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isMusicOk_l)
        {
            ...
            for (i = st; i < order / 2 + 6 && i < notesNum_r; i++)
            {
                ...
                UpdateBlack(sounds_r[i].finger, sounds_r[i].pitch);
             }
        }
    ...
    private void UpdateTranslate()
    {
        ...
        if (trans_order < notesNum_r && (Time.time - inital_time) >= sounds_r4hands[trans_order].start) //start time minus translate time;
        {
    ...
    private void UpdateBlack(int number, int pitch)
    {
        bool black = isBlack(pitch);
        ani.SetBool("black" + number, black);
    }
    private bool isBlack(int pitch)
    {
        int pos = (pitch - 11) % 12;
        return pos == 2 || pos == 4 || pos == 7 || pos == 9 || pos == 11;
    }
